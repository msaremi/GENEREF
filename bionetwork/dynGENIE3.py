...
def dynGENIE3(TS_data, time_points, alpha='from_data', SS_data=None, gene_names=None, regulators='all',
    ...
    if nthreads > 1:
        ...
        for i in range(ngenes):
            ...
            (vi, prediction_score_i, stability_score_i, treeEstimator) = dynGENIE3_single(TS_data, time_points, SS_data,
                                                                                          i, alphas[i], input_idx,
...
def wr_dynGENIE3_single(args):
    return ([args[3],
             dynGENIE3_single(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9],
                              args[10])])
...
def dynGENIE3_single(TS_data, time_points, SS_data, output_idx, alpha, input_idx, tree_method, K, ntrees,
    ...
    for (i, current_timeseries) in enumerate(TS_data):
        ...
        output_vect_time_present[nsamples_count:nsamples_count + nsamples_current] = current_timeseries[:npoints - h,
                                                                                     output_idx]
    ...
    if compute_quality_scores:
        ...
        if sum(importances_by_tree) > 0:
            ...
            stability_score = mean([len(top_by_tree[i].intersection(top_by_tree[j])) for (i, j) in
...
def dynGENIE3_predict_doubleKO(expr_WT, treeEstimators, alpha, gene_names, regulators, KO1_gene, KO2_gene, nTimePoints,
                               deltaT):
    ...
    for t in range(1, nTimePoints + 1):
        new_expr = [(treeEstimators[i].predict(TS_predict[t - 1, input_idx].reshape(1, -1)) - alphas[i] * TS_predict[
            t - 1, i]) * deltaT + TS_predict[t - 1, i] for i in geneidx]
